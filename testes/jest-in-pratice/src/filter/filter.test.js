...
it('filter([1, 2, 3], item => item < 2) should returno [1]', () => {
    expect(filter([1, 2, 3], (item) => item < 2)).to.be.deep.equal([1]);
});

it('filter([1, 2, 3, 5], (item, index) => item ==== index + 1) should returno [1, 2, 3]', () => {
    expect(filter([1, 2, 3, 5], (item, index) => item === index + 1)).to.be.deep.equal([1, 2, 3]);
});

it('filter([1, 2, 3, 2, 3, 5], (item, index, array) => index === array.indexOf(item)) should returno [1, 5]', () => {
    expect(filter([1, 2, 3, 2, 3, 5], (item, index, array) => index === array.indexOf(item))).to.be.deep.equal([1, 2, 3, 5]);
});